...
// Injected by vite.config.ts define — works for both local (.env VITE_GEMINI_API_KEY)
// and Docker builds (GEMINI_API_KEY build-arg).
...
function getApiKey(): string {
  // __GEMINI_KEY__ is injected by vite.config.ts from VITE_GEMINI_API_KEY (local) or GEMINI_API_KEY (Docker)
  if (!__GEMINI_KEY__) throw new Error('GEMINI_API_KEY не задан (см. .env или Docker build-arg)');
  ...
  for (const ver of ['v1beta', 'v1', 'v1alpha']) {
    try {
      ...
      _availableModels = (data.models ?? [])
        ...
        .filter((name: string) => !SKIP_KEYWORDS.some((kw) => name.includes(kw)));
...
export async function extractBatchInsights(
  ...
  // Send only the lightweight fields to save tokens
