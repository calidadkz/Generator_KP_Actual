...
// Категория атома — СВОБОДНАЯ СТРОКА (тема знания, не этап скрипта).
// Единственная зарезервированная тема: 'Формулировки' — атомы видны менеджеру ВСЕГДА.
// Примеры тем: 'Охлаждение', 'Трубки', 'Материалы', 'Цена', 'Гарантия', 'Доставка'.
// Не используй названия этапов скрипта (Открытие, Квалификация и т.д.) — это разные системы.
// Связь атома с этапом = через scriptNode.microPresentationIds[], а не через category.

// Категория ЭТАПА скрипта — фиксированный enum (тип этапа воронки).
...
const AGENT_TOOLS = [
  {
    ...
    input_schema: {
      ...
      properties: {
        category: {
          type: 'string',
          description: 'Фильтр по теме атома. Свободная строка (например: Охлаждение, Цена). Для атомов "всегда видны" используй тему "Формулировки".',
        },
        query: {
          type: 'string',
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        category: {
          type: 'string',
          description: 'Тема атома — свободная строка. Отражает ПРЕДМЕТ знания, не этап скрипта. Примеры: "Охлаждение", "Трубки / Источник", "Материалы", "Цена", "Гарантия", "Сроки поставки". Зарезервировано: "Формулировки" — этот атом будет виден менеджеру ВСЕГДА на любом этапе звонка.',
        },
